(function () {
  ...
  // Defenders never cross this line -- everything from here to the goal
  // line is a safe zone right in front of the keeper, where the field
  // defenders can't reach you. Beat the three of them in the open middle
  // of the pitch, then it's just you against the goalkeeper.
  var SAFE_ZONE_X = GOAL_LINE_X - 90;
  ...
  // ---- Entities ----
  // All three homes sit well short of the safe zone, spread across the
  // open middle of the pitch you have to beat them in.
  function makeDefenders() {
    return [
      { homeX: 280, homeY: 150, x: 280, y: 150 },
      { homeX: 280, homeY: 350, x: 280, y: 350 },
      { homeX: 470, homeY: 250, x: 470, y: 250 }
    ];
  }
  ...
  function resetPositions() {
    ...
    ball = { x: START_X + DRIBBLE_OFFSET, y: START_Y, mode: "dribble", vx: 0, vy: 0 };
  ...
  // Strike the ball in the direction the player is currently facing,
  // sending it off on its own at SHOT_SPEED instead of staying glued to
  // the player. Only has an effect while a ball is being dribbled.
  function shoot() {
    if (ball.mode !== "dribble") return;
    ball.mode = "flying";
    ball.vx = Math.cos(player.facing) * SHOT_SPEED;
    ball.vy = Math.sin(player.facing) * SHOT_SPEED;
  }
  ...
  function update(now) {
    ...
    if (move.x !== 0 || move.y !== 0) {
      ...
      gameStarted = true; // defenders wake up on your first move
    }
    ...
    if (ball.mode === "dribble") {
      ...
      ball.x += ball.vx * dt;
      ball.y += ball.vy * dt;
      if (ball.y < FIELD_TOP) { ball.y = FIELD_TOP; ball.vy = Math.abs(ball.vy); }
      if (ball.y > FIELD_BOTTOM) { ball.y = FIELD_BOTTOM; ball.vy = -Math.abs(ball.vy); }
    ...
    // they amble toward their patrol spot until the ball gets close, then
    // chase it directly, but never into the safe zone in front of goal.
    if (gameStarted) {
      var ENGAGE_DIST = 220;
      defenders.forEach(function (d) {
        var toBallX = ball.x - d.x, toBallY = ball.y - d.y;
        var distToBall = Math.hypot(toBallX, toBallY);
        var targetX, targetY;
        if (ball.x < SAFE_ZONE_X && distToBall < ENGAGE_DIST) {
          targetX = ball.x; targetY = ball.y;
        } else {
          targetX = d.homeX; targetY = d.homeY;
        }
        ...
        d.x = Math.min(d.x, SAFE_ZONE_X); // hard ceiling: can never step into the safe zone
      });
    }
  ...
  function drawPitch() {
    ...
    // Safe zone shading: a strip right in front of the goal where the
    // field defenders never set foot -- only the keeper can stop you there.
    ctx.fillStyle = "rgba(255,255,255,0.12)";
    ctx.fillRect(SAFE_ZONE_X, FIELD_TOP, FIELD_RIGHT - SAFE_ZONE_X, FIELD_BOTTOM - FIELD_TOP);
    ...
    // safe-zone boundary (dashed), with its label
    ctx.save();
    ctx.setLineDash([8, 6]);
    ctx.beginPath();
    ctx.moveTo(SAFE_ZONE_X, FIELD_TOP);
    ctx.lineTo(SAFE_ZONE_X, FIELD_BOTTOM);
    ctx.stroke();
    ctx.restore();
    ctx.save();
    ...
    ctx.translate((SAFE_ZONE_X + FIELD_RIGHT) / 2, H / 2);
